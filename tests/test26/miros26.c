...
#define MAX_PRIO(bitmask_) (32U - __clz(bitmask_))
...
void OS_sched(void) {
    ...
    if (OS_readySet == 0U) { /* idle condition? */
        ...
        OS_next = OS_thread[MAX_PRIO(OS_readySet)];
    }
...
void OS_tick(void) {
    ...
    while (workingSet != 0U) {
        uint8_t n = MAX_PRIO(workingSet);
        uint32_t bit = (1U << (n - 1U));
        Q_ASSERT(OS_thread[n]->timeout != 0U);
        --OS_thread[n]->timeout;
