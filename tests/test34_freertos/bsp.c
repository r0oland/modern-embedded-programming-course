    ...
    */
    ...
    if ((tmp & BTN_SW1) != 0U) {  /* debounced SW1 state changed? */
        if ((buttons.depressed & BTN_SW1) != 0U) { /* is SW1 depressed? */
            /* signal the "button-Pressed" semaphore from ISR */
            xSemaphoreGiveFromISR(BSP_semaPress,
                                  &xHigherPriorityTaskWoken);
        }
        else { /* the button is released */
            /* signal the "button-Released" semaphore from ISR */
            xSemaphoreGiveFromISR(BSP_semaRelease,
                                  &xHigherPriorityTaskWoken);
        }
    }
